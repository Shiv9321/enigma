...
)=>
{
  ...
  const createItem = async () => {
    // Debugging logs for initial data review
    console.log("Attempting to create shipment with the following details:", shipment);
    ...
    if (!shipment.pickupTime) {
        console.error("Pickup Time is required.");
        return; // Stop execution if condition fails
    }
    if (!shipment.distance || isNaN(shipment.distance)) {
        console.error("Valid distance is required.");
        return; // Stop execution if condition fails
    }
    if (!shipment.price || isNaN(shipment.price)) {
        console.error("Valid price is required.");
        return; // Stop execution if condition fails
    }

    // If all conditions are met, attempt to create the shipment
    try {
          ...
          <div className="flex justify-end">
            <button className="p-2 text-gray-400  rounded-md hover:bg-gray-100"
              onClick={ ()=>
              {
                ...
                <svg xmlns="http://www.w3.org/2000/svg" className="w-5 h-5 mx-auto" viewBox="0 0
